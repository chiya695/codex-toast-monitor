...
发布页提供免安装压缩包。开启 Windows 开发者模式后，解压压缩包并运行其中的 `安装-开发者模式.ps1`，脚本会为当前用户注册程序包身份并启动程序。卸载时运行 `卸载-开发者模式.ps1`；卸载不会删除本地日志和飞书配置。程序设置中的“开机自动启动”使用当前用户设置，不需要管理员权限。
